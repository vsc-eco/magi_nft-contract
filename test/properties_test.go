...
func TestMintWithProperties(t *testing.T) {
	...
	getResult := CallContract(t, ct, "getProperties", getPayload, nil, ownerAddress, true, uint(150_000_000), "")
...
func TestMintBatchWithProperties(t *testing.T) {
	...
	result := CallContract(t, ct, "mintBatch", mintPayload, nil, ownerAddress, true, uint(200_000_000), "")
	if result.Ret != `{"success":true}` {
		t.Errorf("Expected success, got %s", result.Ret)
	}
	...
	get1Result := CallContract(t, ct, "getProperties", get1, nil, ownerAddress, true, uint(150_000_000), "")
	if get1Result.Ret != `{"properties":{"color":"red"}}` {
		t.Errorf("Expected batch-p1 properties, got %s", get1Result.Ret)
	}

	get2 := []byte(`{"id":"batch-p2"}`)
	get2Result := CallContract(t, ct, "getProperties", get2, nil, ownerAddress, true, uint(150_000_000), "")
...
func TestSetPropertiesSuccess(t *testing.T) {
	...
	result := CallContract(t, ct, "setProperties", setPayload, nil, ownerAddress, true, uint(150_000_000), "")
	if result.Ret != `{"success":true}` {
		t.Errorf("Expected success, got %s", result.Ret)
	}
...
func TestGetPropertiesAnyoneCanRead(t *testing.T) {
	...
	getResult := CallContract(t, ct, "getProperties", getPayload, nil, "hive:other", true, uint(150_000_000), "")
